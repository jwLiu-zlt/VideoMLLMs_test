...
from rdt_gate.token_prototype import build_frame_token_prototype
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--prototype_pool", choices=["mean", "cls"], default="mean")
...
def main() -> None:
        ...
        payload.update(
            {
                ...
                else torch.from_numpy(prototype.prototype_vector),
                "prototype_vector_shape": None
                if prototype.prototype_vector is None
                else tuple(prototype.prototype_vector.shape),
                "prototype_pool": args.prototype_pool,
                "prototype_keep_cls": args.prototype_keep_cls,
                "deviation_judgments": [judgment.__dict__ for judgment in prototype.judgments],
            }
        )
    ...
    if prototype is not None:
        ...
        _write_prototype_csv(csv_path, prototype.results, prototype.judgments)
    ...
    if prototype is not None:
        ...
        print(
            ...
            f"{None if prototype.prototype_vector is None else tuple(prototype.prototype_vector.shape)}"
        )
        print(f"prototype ready: {prototype.prototype_vector is not None}")
...
def _write_prototype_csv(path: str, rows, judgments) -> None:
